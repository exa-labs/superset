...
10. **Local desktop auth and native agents** - before debugging desktop browser/native-agent UI, read `apps/desktop/docs/LOCAL_AUTH_NATIVE_AGENTS_RUNBOOK.md`. Start the full root dev graph (`bun run dev`), not just `apps/desktop`, when auth, Electric collections, embedded browser routes, or native Devin/Capy depend on the local API. Store Capy/Devin API keys only in the encrypted native-agent credential store, never in source or committed env files.
