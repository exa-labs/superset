...
describe("dashboardFocusIndicatorHints", () => {
	...
	it("hides Vim-only action hints when Vim mode is disabled", () => {
		...
		).toEqual(["⌥K", "↑↓", "↵/Space/h/l", "n/N/p/m/F/e/U/a/x/?"]);
	});
	...
	it("advertises the shortcut that works in the current keyboard mode", () => {
		...
		expect(
			dashboardFocusIndicatorShortcutTitle("Sidebar focus", {
				hints: ["⌥K", "↑↓", "↵/Space/h/l", "n/N/p/m/F/e/U/a/x/?"],
				vimModeEnabled: false,
			}),
		).toBe(
			"Sidebar focus. Keys: Option+K, Up/Down, Enter/Space/h/l, n, N, p, m, F, e, U, a/x, ?. Press ? for full keyboard shortcuts.",
		);
		expect(
			dashboardFocusIndicatorShortcutTitle("Native agent focus", {
				hints: ["Esc", "⌥K", "⌥N/⌥⇧N", "r/o/b/m/F/e/u/U/x/X/f/?"],
				vimModeEnabled: true,
			}),
		).toBe(
			"Native agent focus. Keys: Esc, Option+K, Option+N/Option+Shift+N, r, o, b, m, F, e, u, U, x, X, f, ?. Press ? for full keyboard shortcuts.",
		);
	});
	...
	it("shows visible command and shortcut labels without Vim-only clutter", () => {
		...
		expect(
			dashboardFocusIndicatorVisibleHintLabels(
				["Esc", "⌥K", "⌥N/⌥⇧N", "r/o/b/m/F/e/u/U/x/X/f/?"],
				{
					vimModeEnabled: true,
				},
			),
			...
			"Esc Sidebar",
			"⌥N Unread, ⌥⇧N Read",
			"r Reply, o Browser, b View, m/F Folder, e Rename, u Unread, U Read, x Hide, X Archive, f Hints, ? Map",
		]);
		expect(
